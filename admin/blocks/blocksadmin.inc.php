...
//  $dummyhtml = '<html><head><meta http-equiv="content-type" content="text/html; charset='._CHARSET.'" /><meta http-equiv="content-language" content="'._LANGCODE.'" /><title>'.$xoopsConfig['sitename'].'</title><link rel="stylesheet" type="text/css" media="all" href="'.getcss($xoopsConfig['theme_set']).'" /></head><body><table><tr><th>'.$myblock->getVar('title').'</th></tr><tr><td>'.$myblock->getContent('S', $bctype).'</td></tr></table></body></html>';
  ...
  echo '<a href="../index.php?admin=blocks">'. _AM_BADMIN .'</a>&nbsp;<span style="font-weight:bold;">&raquo;&raquo;</span>&nbsp;'.$block['form_title'].'<br /><br />';
...
//    order_block($bid[$i], $weight[$i], $visible[$i], $side[$i]);
  ...
  redirect_header("../index.php?admin=blocks$query4redirect",1,_AM_DBUPDATED);
  // GIJ end
  exit();
}
...
if ( $op == 'order2' ) {
	...
	$query4redirect = '&dirname=' . urlencode( strip_tags( substr( $_POST['query4redirect'] , 9 ) ) ) ;
	redirect_header("../index.php?admin=blocks$query4redirect",1,_MD_AM_DBUPDATED);
	exit;
}
...
/*  if ( !empty($_POST['bside']) ) { $bside = intval($_POST['bside']); } else { $bside = 0; }
  ...
  update_block($bid, $bside, $bweight, $bvisible, $btitle, $bcontent, $bctype, $bcachetime, $bmodule, $options);*/
	...
	redirect_header('../index.php?admin=blocks',1,$msg);
}
...
if ( $op == 'delete_ok' ) {
  ...
  // delete_block_ok($bid); GIJ imported from blocksadmin.php
		...
		if ( $myblock->getVar('block_type') != 'D' && $myblock->getVar('block_type') != 'C' ) {
			redirect_header('../index.php?admin=blocks',4,'Invalid block');
			exit();
		}
		$myblock->delete();
		...
		redirect_header('../index.php?admin=blocks',1,_AM_DBUPDATED);
		exit();
...
if ( $op == 'edit' ) {
  ...
  // edit_block($bid); GIJ imported from blocksadmin.php
		...
		echo '<a href="../index.php?admin=blocks">'. _AM_BADMIN .'</a>&nbsp;<span style="font-weight:bold;">&raquo;&raquo;</span>&nbsp;'._AM_EDITBLOCK.'<br /><br />';
		include dirname(__FILE__).'/myblockform.php'; //GIJ
		$xoopsGTicket->addTicketXoopsFormElement( $form , __LINE__ , 1800 , 'myblocksadmin' ) ; //GIJ
		$form->display();
...
if ($op == 'clone') {
	...
	echo '<a href="../index.php?admin=blocks">'. _AM_BADMIN .'</a>&nbsp;<span style="font-weight:bold;">&raquo;&raquo;</span>&nbsp;'._AM_CLONEBLOCK.'<br /><br />';
